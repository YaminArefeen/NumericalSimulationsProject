...
im_color_show(:,:,2)=im_color_show(:,:,2)+logical(hurt_health);      % hurt_health purple
im_color_show(:,:,1)=im_color_show(:,:,1)+0*logical(killed_tumor);     % killed_tumor green
im_color_show(:,:,3)=im_color_show(:,:,3)+0*logical(killed_tumor);     % killed_tumor green
im_color_show(:,:,2)=im_color_show(:,:,2)+0.5*logical(killed_tumor);     % killed_tumor green
im_color_show(:,:,1)=im_color_show(:,:,1)+logical(nonkilled_tumor);  % non-killed tumor red
imshow(im_color_show);
title('Evaluation Map','FontSize',16);
