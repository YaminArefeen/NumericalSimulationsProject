function [all_x,t_all] = dyn_trap_withVisualization(x_init,dt,T,f,tolerances_newt,x_ref,...
...
for ii = 1:num_iters %loop while t is less than total desired time
    ...
    fprintf('Time: %d || Newton Steps: %d || Accuracy: %f || Dt: %f\n',...
        t,num_newt_step,accuracy,dt)
...
%     Update time step and new value
   ...
   if visualize
    ...
    %break if desired accuracy achieved
    if(accuracy <= des_acc)
