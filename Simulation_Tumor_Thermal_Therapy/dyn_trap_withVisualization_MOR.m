function [all_x,t_all] = dyn_trap_withVisualization_MOR(x_init,dt,T,f,tolerances_newt,...
        max_dt,T1_image,brainmask,max_temp,mc,interval_steps_showResults,visualize)
...
%     Update time step and new value
    ...
    t_all(ii)=t;
   if mod(ii,interval_steps_showResults)==0
       if visualize
           Temperature_im(brainmask)=mc'*x;
           imshow(T1_image,[0 1],'InitialMag', 'fit'); colormap('gray');
           hold on
